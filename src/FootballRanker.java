...
public class FootballRanker {
    ...
    private double printRootMeanSquareError(List<String> humanRanks,
        ...
        Map<String, Integer> predictedRankMap = new HashMap<>();
        int predictedRank = 1;

        //populate the hashmap with the team & our predicted rank
        for (AllPathsInfo info : paths) {
            predictedRankMap.put(info.getName(), predictedRank++);
        }

        int apRankings = humanRanks.size();
        double sumOfSquaredDifference = 0;
        int numTeams = paths.size();

        for (int i = 0; i < apRankings; i++) {
            String team = humanRanks.get(i);
            int actualRank = i + 1;
            //if the team is not in our top 25 rankings, then we assign one plus the number of teams
            int predRank = predictedRankMap.getOrDefault(team, numTeams + 1);
            int diff = predRank - actualRank;
            sumOfSquaredDifference += diff * diff;
            if (showResults) {
                System.out.println("predicted rank: " + predRank + ", actual rank: " + actualRank +
                        ", " + team);
            }
        }
        double rootMeanSquareError = Math.sqrt(sumOfSquaredDifference / apRankings);
        return Math.round(rootMeanSquareError * 10.0) / 10.0;
    }
