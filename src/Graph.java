...
public class Graph {
    ...
    public void dijkstra(String startName) {
        ...
        clearAll();
        currentStartVertexName = startName;
        //read in the starting vertex
        Vertex startVert = getVertex(startName);
        //initialize the distance to be 0
        ...
        //enqueue the path starting from vertex
        queue.add(new Path(startVert, 0));

        while(!queue.isEmpty()){
            // a. Find the unvisited vertex that currently has the shortest
            // distance from the start (dequeue from the priority queue).
            Path curPath = queue.remove();
            Vertex curVertex = curPath.dest;
            //if the vertex has already been visited, we know that (bc of the priority queue) that
            //we have reached it through the shortest possible path
            if (curVertex.scratch != 1) {
                // b. Mark the current vertex as visited.
                curVertex.scratch = 1;
                for (Edge e : curVertex.adjacent) {
                    // c. Update the neighbors if a shorter path has been found.
                    double pathTotal = curPath.weightedCostOfPath + e.cost;
                    //this will always bw true unless we have already found a faster path, remember
                    //that vertexes distances are set to INFINITY at the start
                    if (e.dest.weightedCostFromStartVertex > pathTotal) {
                        //update distance
                        e.dest.weightedCostFromStartVertex = pathTotal;
                        //update prev vertex
                        e.dest.prev = curVertex;
                        //update the edge count
                        e.dest.numEdgesFromStartVertex = curVertex.numEdgesFromStartVertex + 1;
                    }
                    // d. Enqueue the path to the priority queue.
                    //only if we have not already visited the vertex
                    if (e.dest.scratch != 1) {
                        queue.add(new Path(e.dest, pathTotal));
                    }
                }
            }
        }
    ...
    public void findAllPaths(boolean weighted) {
        ...
        //need to clear path info first
        for (Vertex toBeCleared : vertexSet){
            toBeCleared.clearPathInfo();
        }

        //now go through and fill in info
        for (Vertex startVert : vertexSet) {
            Vertex start = startVert;

            if (weighted){
                //updates start vert var
                dijkstra(start.name);
            } else {
                //updates start vert var
            ...
            //need to go through every other vertex in graph and see if there is a valid path to
            for (Vertex otherVertex : vertexSet){
                //make sure we arnt counting the vertex we are searching from
                //also make sure that there is actually a path to it (through the .prev check)
                if (startVert != otherVertex && otherVertex.prev != null){
                    startVert.numVertexConnected++;
                    //count this vertex
                    startVert.totalUnweightedPathLength += otherVertex.numEdgesFromStartVertex;
                    //bc unweighted, path cost == 1
                    startVert.totalWeightedPathLength += otherVertex.weightedCostFromStartVertex;
                    //update the total w cost per reqs


                    //need to now check if this is the longest valid path
                    //from start -> param
                    Path curPath = getPath(otherVertex.name);
                    //now need to check if this path is longer than our current longest path
                    if (longest == null || (weighted && curPath.weightedCostOfPath >
                            longest.weightedCostOfPath) || (!weighted &&
                            curPath.getNumVerticesInPath() > longest.getNumVerticesInPath())){
                        longest = curPath;
                    }
                }
            }
        }
        //update all paths found var
        allPathsFound = true;
    }
